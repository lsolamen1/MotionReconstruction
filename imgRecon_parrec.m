function [flt_data,parms,dims] = imgRecon_parrec(parfile,out_fmt,verboseflag);
% [FLT_DATA,PARMS,DIMS= = IMGRECON_PARREC(PARFILE,OUT_FMT,VERBOSEFLAG)
...
%         dims: [stride nline nslice necho nphase ntype ndyn ngrad]
%
% Can process both V3 and V4 and V4.1 types of PAR files automatically
...
% Modified by   J. Chris Gatenby, PhD
%               Vanderbilt University Institute of Imaging Science
%               Date: 1/27/2005
%                   1, input of parfile name directly
%                   2, Data format output selection
%                   3, Platform independent file conversion
%               Date: 2/14/2005 
%                   1, correctly parse multiple echoes

% Modified by   Huifang Wang, PhD
%               Dartmouth College, Thayer School of Engineering
%               Date: 11/28/2006, 4/17/2007 for r2
%                   1, Add V4.1 type template 
%                   2, Add one more dimension for gradient orients for
%                   diffusion
%==========================================================================
...
% figure out if V3, V4 or V4.1 PAR files
test_key1 = '.    Image pixel size [8 or 16 bits]    :';
test_key2 = '.    Max. number of diffusion values    :';
if strmatch(test_key1,geninfo); % only V3 has that key in the headers
    ver = 'V3';
    template=get_template_v3;
elseif strmatch(test_key2,geninfo); % only V4.1 has that key in the headers
    ver = 'V4.1';
    template=get_template_v41;
else
    ver = 'V4';
...
% ngrad = parms.max_gradient_orients+1;
...
switch(ver)
    case {'V3'}, pixel_bits = parms.pixel_bits; 
        ngrad=1;
    case {'V4'}, pixel_bits = parms.tags(1,8); 
        ngrad=1;  % assume same for all imgs
    case {'V4.1'}, pixel_bits = parms.tags(1,8);
            if ( parms.diffusion == 0 )
                ngrad = parms.max_gradient_orients;
            else
                ngrad = parms.max_gradient_orients+1;
            end        
end
...
% dims = [stride nline nslice necho nphase ntype ndyn ngrad+1];
dims = [stride nline nslice necho nphase ntype ndyn ngrad];
...
for I  = 1:nimg
    ...
    seq = parms.tags(I,6);
    rec = parms.tags(I,7);
    grad = I./(nslice.*necho.*nphase.*ntype.*ndyn);
    grad = ceil(grad);
    ...
    image_data(:,:,slice,echo,phase,type_idx,dyn,grad) = img;
end
return;
...
function [template] = get_template_v41;    % header information for V4.1 PAR files
